...
- Training code smoke-tested on CPU: 203.6M params, loss magnitudes sane
  (initial attn CE ≈ ln 400), grads flow. 6621/6631 train pairs and 60/60
  eval pairs have usable depth (test_* frames ship zero depth).
...
- `scripts/r2_driver.sh` launched detached: waits for the R1 decoder-scale
  sweep to release the GPUs, then desc precompute → 30-step GPU smoke run
  → match + joint arms in parallel → recon arm. Logs in `results/r2_v0/`.
